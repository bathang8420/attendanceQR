...
if(isset($_POST["action"]))
{
	if($_POST["action"] == "fetch")
	{
		...
		foreach($result as $row)
		{
			...
			$sub_array[] = '
			<button type="button" name="view_teacher" class="btn btn-info btn-sm view_teacher" id="'.$row["teacher_id"].'"><i class="fas fa-eye"></i></button>
			<button type="button" name="edit_teacher" class="btn btn-primary btn-sm edit_teacher" id="'.$row["teacher_id"].'"><i class="fas fa-edit"></i></button>
			<button type="button" name="delete_teacher" class="btn btn-danger btn-sm delete_teacher" id="'.$row["teacher_id"].'"><i class="fas fa-trash"></i></button>
			';
	...
	if($_POST["action"] == 'Add' || $_POST["action"] == "Edit")
	{
		...
		else
		{
			if($_POST["action"] == 'Add')
			{
				...
				(teacher_name, teacher_address, teacher_emailid, teacher_password, teacher_doj, teacher_image) 
				SELECT * FROM (SELECT :teacher_name, :teacher_address, :teacher_emailid, :teacher_password, :teacher_doj, :teacher_image) as temp 
				WHERE NOT EXISTS (
	...
	{
		$query = "
		SELECT * FROM tbl_teacher WHERE tbl_teacher.teacher_id = '".$_POST["teacher_id"]."'
		";
		$statement = $connect->prepare($query);
				...
				<div class="col-md-9">
					<table class="table">
						...
						<tr>
							...
							<td>'.date("d/m/Y",strtotime($row["teacher_doj"])).'</td>
						</tr>
					</table>
				</div>
				';
